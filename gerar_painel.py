...
ALERTAS = ((("FATURA",), "FATURA"),
           ...
           # 10/09/2026. Nota cancelada/substituida (na SEFAZ ou no relatorio NFS
           # STATUS) que ESTA lancada na SF1 -- pede estorno. Vermelho como a
           # removida: nao e atraso, e lancamento que nao devia existir.
           ((sefaz.PREFIXO_CANCELADA,), "CANCELADA E LANÇADA"),
...
COMPACTA_SEFAZ_SF1 = [
    ...
    ("Títulos",           "",       "",           3.5),
    # 09/09/2026: os titulos da SF1 que podem ser esta nota mesmo com o numero
    # da NF nao batendo (fornecedor + valor ou emissao). A lista e copiavel; o
    # POR QUE de cada um abre no quadro da coluna do lado.
    ("Títulos Prováveis na SF1",      "boleto", "",   15),
    ("Por que estes títulos prováveis", "boleto", "",  3),
...
ROTULOS_PLANILHA_SEFAZ_SF1 = {
    ...
    "Títulos Prováveis na SF1": "Títulos da SF1 que podem ser esta nota (fornecedor + valor ou emissão)",
    "Por que estes títulos prováveis": "Por que cada título é provável",
...
COMPACTA_PEDIDOS = [
    ...
    ("Lançada na SF1",    "",       "",             6.5),
    # 09/09/2026, pedido do usuario. COLADAS no veredito de lancamento de
    # proposito: a pergunta que a lista responde e a daquela coluna ("a nota
    # esta lancada?"), nao a do pedido. Sao os titulos da SF1 daquele
    # FORNECEDOR com o mesmo valor ou a mesma emissao -- o numero da NF nao
    # entra, porque e ele que nao bate. Ver csf1.possibilidades().
    ("Títulos Prováveis na SF1",      "",       "",   15),
    # o "ver ▾" que abre o quadro com o por que de cada titulo provavel
    ("Por que estes títulos prováveis", "",     "",    3),
    ("Origem",            "",       "",             4),
    ("Nº NF",             "titulo", "NF",           4.5),
    ...
    # 10/09/2026. O relatorio NFS STATUS (aba da SEFAZ.xlsx): Exportada /
    # Disponível = a integracao da NFS-e com o TOTVS segundo o portal, mais a
    # situacao na prefeitura quando diverge da SEFAZ. Filtravel (`pills`).
    ("NFS STATUS",        "titulo", "",             6),
    ...
    # o icone "i" que abre o quadro do POR QUE -- coluna propria, colada na lista
    ("@porque_pc",          "boleto", "PEDIDO",     2.5),
...
ROTULOS_PLANILHA_PEDIDOS = {
    ...
    "NFS STATUS": "Integração ERP e situação na prefeitura (aba NFS STATUS da SEFAZ.xlsx)",
    "Emissão": "Emissão (SEFAZ)",
    "Emitente": "Emitente/Prestador (SEFAZ)",
    "Nome Fantasia": "Nome Fantasia do Emitente",
    "Destinatário": "Destinatário/Tomador (a nossa empresa)",
...
ABAS = {
    ...
    "sefaz": {
        ...
        "selos": {"NF-e": "nfe", "NFS-e": "nfse", "CT-e": "nfse", "NFS STATUS": "nfse"},
    ...
    "sefaz_sf1": {
        ...
        "copiar_extra": {"Nº NF": "", "Destinatário": "", "CNPJ Destinatário": "",
                         "Títulos Prováveis na SF1": ""},
        # 09/09/2026: o POR QUE dos titulos provaveis e texto de varias linhas --
        # na celula cabe o "ver ▾" e o conteudo abre no quadro, com o nome da
        # coluna no titulo. ⚠ FORA DA BUSCA pelo mesmo motivo do JSON dos PC
        # provaveis: sao as MESMAS palavras em toda linha ("mesmo CNPJ", "valor
        # exato") e digitar qualquer uma devolveria a aba inteira. O que
        # interessa buscar -- os numeros dos titulos -- esta na coluna visivel.
        "texto_longo": {"Por que estes títulos prováveis"},
        "fora_da_busca": ("Por que estes títulos prováveis",),
        "col_alerta": "Alerta",
    ...
    "pedidos": {
        ...
        "pills": ["Origem", "Lançada na SF1", "Controle Ap.", "Status da Nota",
                  "Manifestação", "NFS STATUS", "Filial do PC"],
        ...
        # 09/09/2026: o POR QUE dos titulos provaveis da SF1 entra aqui -- a
        # celula vira "ver ▾" e o texto abre no quadro, como a Info Comp.
        "texto_longo": {"Informações", "Por que estes títulos prováveis"},
        ...
        # 04/09/2026: a coluna dos PC provaveis ganha quadro proprio, com o
        # porque de cada pedido. `col_provaveis` e a que aparece na tela (vira
        # `papel: "provaveis"`); `col_provaveis_motivos` e a escondida que
        # carrega o JSON -- e que fica fora da busca pelo mesmo motivo.
        "col_provaveis": "Numero PC Prováveis",
        "col_provaveis_motivos": pedidos_sefaz.MOTIVOS_PROVAVEIS,
        "fora_da_busca": (pedidos_sefaz.MOTIVOS_PROVAVEIS,
                          pedidos_sefaz.PORQUE_PROVAVEIS_SF1),
    },
}
...
ROTULOS_COMPACTA = {"@delta_valor": "Δ valor", "@delta_dias": "Δ dias",
                    ...
                    "@alerta_tipo": "Confere?",
                    "@porque_pc": "?",
                    # o cabecalho e uma linha so: o nome inteiro vai no .xlsx
                    # e no quadro (ROTULOS_PLANILHA_*), aqui fica o curto.
                    "Títulos Prováveis na SF1": "Títulos prováveis",
                    "Por que estes títulos prováveis": "Por quê",
...
def montar_aba(cabecalho: list[str], brutas: list, cfg: dict):
    ...
    for bruta in brutas:
        ...
        registros.append({
            ...
            # o quadro do POR QUE dos PC provaveis -- {} nas abas que nao o declaram
            "pcprov": montar_provaveis(origem, cfg),
            ...
            # ⚠ O JSON dos motivos FICA DE FORA da busca: sao centenas de
            # caracteres por linha com as MESMAS palavras em todas ("Fornecedor
            # confere", "Pedido EM ABERTO"), e digitar qualquer uma devolveria a
            # aba inteira. O que interessa buscar (os numeros dos PC) esta na
            # coluna visivel, que continua entrando.
            "busca": " ".join(texto(v) for c, v in origem.items()
                              if texto(v) and c not in cfg.get("fora_da_busca", ())).lower(),
        })
...
TIPOS_VIRTUAIS = {
    ...
    # 05/09/2026: o icone que abre o quadro do POR QUE dos PC provaveis. Coluna
    # PROPRIA, a pedido ("crie uma coluna nova ao lado para incluir esse icone"):
    # dentro da celula de copiar ele disputava espaco com o botao e sumia.
    "@porque_pc": "porque_pc",
}
    ...
    for rotulo, lado, grupo, largura in definicao:
        ...
        colunas.append({
            ...
            "papel": ("status" if rotulo == cfg["col_status"]
                      ...
                      # quadro flutuante
                      # 04/09/2026: a celula dos PC provaveis abre um quadro com o
                      # porque de cada pedido. Pelo PAPEL, como as outras: o
                      # navegador nao precisa saber o nome da coluna.
                      else "provaveis" if rotulo == cfg.get("col_provaveis")
...
def montar_provaveis(origem: dict, cfg: dict) -> dict:
    """O quadro do POR QUE da coluna `Numero PC Prováveis` (04/09/2026).

    Ele: "inclua na coluna numero pc provaveis o porque voce associou aqueles PC
    com uma janela no modelo do print" -- o print e o quadro do selo da Situacao.

    A coluna de origem e ESCONDIDA (esta no CABECALHO do pedidos_sefaz e nao no
    COMPACTA_PEDIDOS): ela carrega o JSON com um item por pedido. ⚠ JSON quebrado
    nao pode derrubar o painel inteiro por causa de um quadro -- devolve vazio, a
    celula continua na tela e so o quadro deixa de abrir.
    """
    coluna = cfg.get("col_provaveis_motivos")
    if not coluna:
        return {}
    bruto = texto(origem.get(coluna))
    if not bruto:
        return {}
    try:
...
def gerar(base: Path, saida: Path, base_pendentes: Path | None = None,
          ...
          base_manifestacao: Path | None = None,
          base_premissa2: Path | None = None) -> dict:
    if not base.exists():
    ...
    if not caminho_sefaz.exists():
        ...
        # 04/09/2026. PREMISSA 2: as notas de MESMA PREMISSA - ORGANIZADA.xlsx que
        # a SEFAZ.xlsx NAO tem entram como LINHAS, com Origem "PREMISSA 2", e
        # seguem o mesmo caminho das outras (SF1, pedidos). Entra AQUI, e nao
        # antes: (1) DEPOIS do `ler_com_historico`, porque estas linhas nao
        # podem ir para o sefaz_historico.json -- a SEFAZ e acumulativa e o
        # historico existe para acusar nota que SUMIU dela; nota que nunca
        # esteve la viraria "removida" na rodada em que o arquivo mudasse;
        # (2) DEPOIS do `manifestacao.anotar`, porque a manifestacao destas
        # linhas vem do proprio arquivo e o anotar as marcaria "NAO ENCONTRADA"
        # (a MESMA PREMISSA.xlsx, csv, pode nao ter as mesmas notas). Falhar
        # aqui nao derruba o painel: avisa e segue sem elas.
        caminho_p2 = base_premissa2 or premissa2.BASE_PADRAO
        try:
        ...
        # de NFS-e nao tem. Mesma posicao da PREMISSA 2, pelos mesmos motivos.
        st_resumo = None
        try:
            novas_st, st_resumo = nfs_status.anotar(linhas_sefaz, empresas_bioflor,
                                                    caminho_sefaz)
            linhas_sefaz.extend(novas_st)
            periodo = (f" | período {st_resumo['de']} a {st_resumo['ate']}"
                       if st_resumo.get("de") else "")
            print(f"           NFS STATUS ({st_resumo['aba']}): {st_resumo['linhas']} linhas "
                  f"({st_resumo['repetidas']} repetidas) | {st_resumo['registros']} notas | "
                  f"{st_resumo['ja_na_sefaz']} já na SEFAZ | {st_resumo['novas']} NOVAS | "
                  f"{st_resumo['divergentes']} com situação divergente{periodo}")
            print("           " + " | ".join(f"{n} {s}" for s, n in st_resumo["anotadas"].items()))
        except Exception as exc:  # noqa: BLE001 - aba ausente/renomeada/mudada
        ...
        if caminho_sf1.exists():
            ...
            if cruz_sefaz_resumo.get("canceladas"):
                print(f"           canceladas/substituídas: {cruz_sefaz_resumo['canceladas']} "
                      f"notas (fora dos alertas de lançamento) | "
                      f"{cruz_sefaz_resumo['canceladas_lancadas']} delas LANÇADAS na SF1 → alerta")
        ...
        sefaz_resumo["nfs_status"] = st_resumo
        sefaz_resumo["cte"] = cte_resumo
        if brutas:
...
def main() -> int:
    ...
    parser.add_argument("--base-premissa2", type=Path, default=premissa2.BASE_PADRAO,
                        help="MESMA PREMISSA - ORGANIZADA.xlsx (notas que a SEFAZ.xlsx "
                             "nao tem entram no painel como Origem PREMISSA 2)")
    ...
    for aba in dados["abas"]:
        ...
        # ⚠ `.get("fora")` e nao `["fora"]`: quando a SE2 nao pode ser lida o
        # `meta_se2` devolve so o mapa de tipos, DE PROPOSITO (o navegador so
        # corta linha quando `fora` existe). Com o colchete, o resumo estourava
        # com KeyError depois de o painel inteiro estar montado -- a geracao
        # morria no ultimo print e nada era publicado.
        if conferido and conferido.get("fora") is not None:
