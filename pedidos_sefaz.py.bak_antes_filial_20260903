...
PESO_TEXTO_FORTE = 128
PESO_TEXTO_FRACO = 64
PESO_CNPJ = 32
PESO_VALOR = 16       # total do pedido = total da nota
PESO_VALOR_ITEM = 8   # total da nota = valor de um item (entrega parcial)
...
def chaves_do_fornecedor(nota: dict, pedidos: dict[str, sc7.Pedido],
                         ...
                         por_nome: dict[str, list[str]]) -> list[str]:
...
def analise_agrupada(linhas: list[dict], pedidos: dict[str, sc7.Pedido],
    ...
    for chave, pedido in pedidos.items():
        if chave in pcs_usados or not pedido.vlr:
            continue
        for raiz in pedido.fornecedores:
            por_forn_pedidos.setdefault(("cnpj", raiz), []).append(pedido)
        for nome in pedido.nomes:
            por_forn_pedidos.setdefault(("nome", nome), []).append(pedido)
    ...
    def balde_da_nota(linha: dict) -> tuple:
        ...
        if por_forn_pedidos.get(("cnpj", raiz)):
            return ("cnpj", raiz)
